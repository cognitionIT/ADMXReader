...
# Include a Row with policy information (and 1-Check for existing policy valueName ; 2-start a new row after this
                ...
                #region retrieve policy valueName attribute information and log a row for policy information (linked to the enabledValue and disabledValue subelements)
                If ($policy.valueName -ne $null)
                {
                    ...
                    If ($policy.enabledValue -ne $null)
                    {
                        ...
                        Switch ($polEnabledValueType)
                        {
                            "string"
                            {
                                $polPossibleValues = ($policy.enabledValue.string + " (Enabled)")
                            }
                            "decimal"
                            {
                                $polPossibleValues = ($policy.enabledValue.decimal.value.ToString() + " (Enabled)")
                            }
                            default
                            {
                                $polPossibleValues = ""
...
Write-Host "enabledList node found." -ForegroundColor Cyan
...
Write-Host "disabledList node found." -ForegroundColor Cyan
